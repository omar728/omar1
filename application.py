...
from flask import (session as login_session,random,string,
json,httplib2,requests)
...
def createUser(login_session):
                        newUser = User(name=login_session['username'],  # noqa
...
category = session.query(Category).filter_by(id = catalog_id).first()
    ...
    categoryItems = session.query(CategoryItem).filter_by(category_id = catalog_id).all()  # noqa

	# Get count of category items
	categoryItemsCount = session.query(CategoryItem).filter_by(category_id = catalog_id).count()  # noqa
s
	return render_template('category.html', categories
    = categories, categoryItems = categoryItems,
...
def editCategoryItem(catalog_id, item_id):
	...
	if request.method == 'POST':
        ...
        catalog_id = categoryItem.category_id ,item_id = categoryItem.id))
	else:
		return render_template('editCategoryItem.html',
        categories = categories, categoryItem = categoryItem)
